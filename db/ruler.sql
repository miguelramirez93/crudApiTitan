...
INSERT INTO predicado VALUES (134, 1, 'valor_pago(X,V,P):-valor_contrato(X,Y), duracion_contrato(X,D,V), R is Y / D, P is (R rnd 0).', 'a', 1);
INSERT INTO predicado VALUES (133, 1, 'valor_pago_neto(X,Y,P,V,L,L2):-valor_pago(X,P,V),valores(X,descuento,P,L),descuentos_ley(X,T,P,L2),unir(L,L2,LS),total_descuentos(LS,D),R is V - D, Y is (R rnd 0).', 'Hace el calculo de el total a pagar al empleado tomenado en cuenta novedades y descu8entos de ley', 1);
INSERT INTO predicado VALUES (113, 1, 'base_desc_ley(X,Y,P,rete384):- valor_pago(X,P,R), Y is (R rnd 0).', 'Calculo de la base para cada descuento', 1);
